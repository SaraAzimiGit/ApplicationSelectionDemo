...
with appSelection:
    ...
    info_app_1={'Occupancy_Resolution':['Tracking','Tracking','Count','Identity','Identity','Identity','Count','Presence','Count','Presence','Presence','Count','Count','Count'],
              'Spatial_Resolution':['Room','Room','Floor','Room','Room','Floor','Room','Floor','Floor','Room','Workstation','Floor','Floor','Floor'],
    ...
    df=pd.DataFrame(data=info_app,columns=columns,index=['Emergency evacuation and rescue','Disease control','Space utilization rate','Access control','Intrusion detection',
        'Surveillance','Smart cleaning','Heating/cooling control','Demand control ventilation','Lighting control','Plug load control','Heating/cooling energy usage','Lighting energy usage','Plug load energy usage'])
    ...
    info_sensor_1 = {
        'Occupancy_Resolution': ['Presence', 'Count', 'Identity', 'Tracking', 'Tracking', 'Count', 'Presence', 'Count','Presence'],
        'Spatial_Resolution': ['Workstation', 'Workstation', 'Workstation', 'Workstation', 'Room', 'Room', 'Room', 'Room','Room'],
    ...
    with st.form(key="Selecting columns"):
        ...
        if submit_button:
            ...
            if Menu_Choices == "Select combination of sensors based on application criteria":
                ...
                min_table = df_0.loc[(min_occ_res <= df_0['Occupancy_Resolution']) & (min_acc <= df_0['Accuracy']) & (
                            min_spatial_res <= df_0['Spatial_Resolution'])]
